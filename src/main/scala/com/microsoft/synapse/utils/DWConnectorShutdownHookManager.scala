...
object DWConnectorShutdownHookManager extends Logging {
  ...
  def addDWConnectorShutdownHook(conf: Configuration):Unit = {
    var shutdownHook: AnyRef = null
    try {
      logInfo(s"Adding shutdownhook to delete stagingtables and directories")
      shutdownHook = DWConnectorShutdownHookManager.addShutdownHook(DWConnectorShutdownHookManager.TEMP_DIR_SHUTDOWN_PRIORITY)({ () =>
        DWConnectorShutdownHookManager.shutdownDeletePaths.toArray.foreach { dirPath =>
          try {
            logInfo("Deleting directory " + dirPath)
            DWConnectorShutdownHookManager.deleteFileRecursive(conf, dirPath)
          } catch {
            case e: Exception =>
              throw e
          }
      ...
      case e: Exception =>
        logDebug(s"Shutdown hook failed with error:", e)
    }
  }

  def deleteFileRecursive(configuration: Configuration, file: String):Unit = {
    val outputPath = new Path(file)
    val fs = outputPath.getFileSystem(configuration)
    if(fs.exists(outputPath)) {
      fs.delete(outputPath, true)
      logInfo(s"$outputPath deleted")
    }
  }
  ...
  def registerStagingDeleteDir(file: String): Unit = {
    shutdownDeletePaths.synchronized {
      shutdownDeletePaths += file
    }
  }
